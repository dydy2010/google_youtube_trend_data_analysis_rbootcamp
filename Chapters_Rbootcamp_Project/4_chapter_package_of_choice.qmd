...
The core function of the package is: gtrends(), which sends queries to the Google Trends API and returns the result.

## Main Parameters

Some of the most relevant arguments that we can specify in `gtrends()` are:
...
-   **`interest_by_region`**\
    Regional breakdown (e.g., states, cities, DMA) depending on `geo` parameter.
...
In summary, the package **gtrendsR** is a powerful tool that allows us importing Google Trends
data directly into R.\
With **gtrendsR**, we can:
...
## Interpretating Google Trend Data

The **values from 0 to 100** on Google Trends represent **normalized,
...
peak value**. For example, a value of 0 means that there is insufficient data or that the search term has a very low search volume.
...
1.  **Normalization**: Google Trends normalizes the data on a scale from
    ...
    queries.
2.  **Relative values**: The values are not absolute quantities, but
...
Google is restricting the download-limit of Google Trends data. Sometimes
...
```{r load gtrends tesla, echo=FALSE, warning=FALSE}
library(here)
...
# )
# 
# save(res, file = here::here("Chapters_Rbootcamp_Project","gtrends_Tesla_world.RData"))

load(here::here("Chapters_Rbootcamp_Project","gtrends_Tesla_world.RData"))

```

### Prepare the `interest_by_country` data in gtrends 

The `interest_by_country` dataframe, which contains geographic search data, required two steps before it could be used in analysis.  

First, We have to adjust some location iso-codes so they fit to the map data.
Second, the main metric 'hits' contains '\<1', which makes the column a character string. We replace \<1 and NA with 0 and set the column to numeric.

```{r interest_by_country preparation,  echo=TRUE, warning=FALSE}
...
```{r get world map data, echo=TRUE, warning=FALSE}
#| code-fold: true   # hide/unhide code chunk
#| code-summary: "Show/Hide the code" 
...
Additionally we specify a color coding from bright to dark blues.The map shows the highest interest in Norway, where electric cars appear to enjoy significant tax advantages. 

```{r plot produce map, echo=FALSE, warning=FALSE, fig.width=8, fig.height=5}
#| code-fold: true   # hide/unhide code chunk
#| code-summary: "Show/Hide the code" 
