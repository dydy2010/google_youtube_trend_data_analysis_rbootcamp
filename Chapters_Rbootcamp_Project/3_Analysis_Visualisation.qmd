...
### Google Search Data Comparison Graph 1: Overall Search Interest Comparison 
*v1(Line Chart)*
...
This part of the big picture is slightly scaled down to iPhone, we would like to have one graph that includes Youtube and Google data in the meantime, and focuses on one product pattern, e.g., the Hype Cycle around product event like the iPhone X lauch in Nov 2017. 

To realize that, 2 versions of the graphs are constructed (180 days next to event). 
In v1, 2 sections of visualisation were drawn, for youtube and google data. The Y axis are the youtube and google engagement and the X axis are the time spans.
...
```
*Interpretation of Google Search Interest Trends*
* Google search interest hype lasted more than 3 months
  From November to February. Even though data before Nov is missing, we could see that starting from           November,the search interest already had already risen and peaked. It indicates that the launch event news   had a great impact on iPhone Google search interest and the elevated interest lasted at least 3 months.
...
* Peak views in December
  Youtube trending videos about iPhone started to emerge around Mid November after the iPhone X launch.
  And the total views of these videos have peaked in December with a daily total view reaching just over 175M   per day. After that there is a deep decline, until a small spike around March. The hype on Youtube will be   short lived and slightly scattered, only lasting about 1 months.

* Youtube strategy for client
  Apple can start to focus on allocating marketing resources on Youtube right after the launch event of a      major product, especially the first month after the event. Continuous marketing effort on Youtube after      that is optional.
  
  


### Graph 3: The iPhone Hype Cycle 
To discover more about the big picture, using iPhone as an example, we put Google search interest on y axis and Youtube views on x axis. The red line shows the correlation of Google interest and Youtube views. 
...
The scatterplots reveals a dispersed data points, and the red line seems to show a slight correlation.
However is it really correlated as our instinct tells us? The result is as shown `r correlation_rounded`. As we know, ±0.0 to ±0.3 equals very weak or no correlation.

We can safely say, even though counter intuitive, while there is a slight tendency for YouTube views and Google searches to move together, the relationship is not strong or predictable. High volumes of YouTube views on a given day were very weakly associated with high levels of Google search interest.

*Platform Insight for client*
The above suggests user behavior on these two platforms was largely independent during the iPhone X launch period. It is beneficial for the client to work with the 2 platforms with independent approach and strategy, for better result.
