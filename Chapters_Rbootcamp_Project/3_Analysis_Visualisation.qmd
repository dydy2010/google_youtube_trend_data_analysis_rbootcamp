...
# Use the same date format specification as before
...
<br> Graph 1: Overall Search Interest Comparison (Facet Chart) v2
...
<br> Graph 3: Distribution of Views v1 Box Plot, doesnt work well with
single data point per day somehow Compare the distribution of daily
views for Tesla vs. iPhone vs. Rolex videos.
...
<br> Graph 3: Distribution of Engagement v2 Violin Plot, doesnt work
well, due to single data point per day somehow Compare the distribution
of daily views for iPhone vs. Rolex videos.
...
<br> Graph 3: Distribution of Engagement v3 Ridge Plot ridge plot is a
modern and visually compelling way to compare distributions. It is
especially effective data that shows peaks and valleys Compare the
distribution of daily views for iPhone vs. Rolex videos.
...
Graph 4: The iPhone Hype Cycle (Dual-Axis Chart) v1 210-day window
around the iPhone X launch.
...
Graph 4: The iPhone Hype Cycle (Separate Chart) v2 210-day window around
the iPhone X launch.
...
## Google Search Trend Analysis (Daniel's part)
...
## Model Anlaysis: fitting a linear regression model and visualize it
...
should not forget that YouTube and Google Searches are primarily about
**content**. What is actually the content of the videos watched in
...
of the content based on all tags that were given to the videos. We think
knowing what the content of the videos is, is as important as knowing
how often they have been watched.
...
we have to restrict the number of words to 500k. And it is even possible
to **define the form of the wordcloud based on a picture**. We use the
apple logo (a png file with the logo in black on a white background) and
`wordcloud2` will try to fill the space used by the Apple logo. This is
definitely something we will try out the next time we do a wordcloud.
