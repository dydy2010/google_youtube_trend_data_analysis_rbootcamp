...
country_data <- res$interest_by_country %>%
  mutate(
    ...
    hits  = suppressWarnings(as.numeric(ifelse(hits == "<1", 0, hits)))
  ) %>%
  select(iso2c, iso3c, country = location, hits, keyword) %>%
  arrange(desc(hits))

write.csv(country_data, "./raw_data_from_google/gtrends_Tesla_all_countries.csv", row.names = FALSE)
...
world_trends <- world_sf %>%
  left_join(country_data, by = c("iso_a3" = "iso3c"))
